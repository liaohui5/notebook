...
// 这样的功能, 将 str 的只读引用传入, 但是并不移交所有权
// 传入引用但是不移交所有权的这个操作就叫做 "借用"
...
在 c 语言中称之为 `指针变量`, 那么在 Rust 中就叫做 `引用`

> 什么是只读引用和可变引用?

Rust 中变量默认就是只读的, 需要使用 `mut` 关键字才可以修改
那么引用也是同样的道理, 默认是只读的, 需要使用 `&mut` 关键才可以修改
...
传入值的引用(指针)而不是值本身就叫做 `借用`

> 什么是只读借用和可变借用

引用默认是只读的, 所以传入的时候就是只读的, 那么这个借用就称之为 `只读借用`
如果传入的引用是可变的, 那么这个借用就可以称之为 `可变借用`
...
fn main() {
    let s = String::from("hello");
    let len = get_str_len(&s); // 传入只读引用, 类似指针
    println!("str = {s}");
    println!("len = {len}");
}
```
...
1. 在任何时间, 同一个作用域下, 要么只能有一个可变引用但可以有多个不可变引用
...
fn main() {
    ...
    // 这样是没有问题的, 因为后续没有再访问 mr4 了
    let mr5 = &mut s;
    println!("mr5={mr5}");

    // println!("mr4={mr4}");
    // 不能在此处访问mr4,因为所有权已经被移交了
    // 也就是说, 当一个变量失去数据所有权之后就无法再访问数据了

    // 就好比我现在有一台电脑,
    // 我借给别人用, 我还可以再要回来用, 因为所有权是我的
    // 但是我卖给别人, 我就无法再用了, 因为所有权不是我的
}
```
