## 介绍

CSS 也和 js 中一样, 有很多实现特定功能的内置的函数, 我记录下特别常用的函数,
要查看更多CSS函数文档, [推荐阅读MDN文档](https://developer.mozilla.org/zh-CN/docs/Web/CSS/CSS_Functions)

## 转换函数

- [translate](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform-function/translate)
- [translateX](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform-function/translateX)
- [translateY](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform-function/translateY)
- [translate3d](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform-function/translate3d)

## 旋转函数

- [rotate](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform-function/rotate)
- [rotateX](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform-function/rotateX)
- [rotateY](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform-function/rotateY)
- [rotateZ](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform-function/rotateZ)
- [rotate3d](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform-function/rotate3d)

## 缩放函数

- [scaleX](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform-function/scaleX)
- [scaleY](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform-function/scaleY)
- [scaleZ](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform-function/scaleZ)
- [scale](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform-function/scale)
- [scale3d](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform-function/scale3d)

## 倾斜函数

- [skewX](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform-function/skewX)
- [skewY](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform-function/skewY)
- [skew](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform-function/skew)

## 透视函数

- [perspective](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform-function/perspective)

## 数学函数

- [calc](https://developer.mozilla.org/zh-CN/docs/Web/CSS/calc)
- [min](https://developer.mozilla.org/zh-CN/docs/Web/CSS/min)
- [max](https://developer.mozilla.org/zh-CN/docs/Web/CSS/max)
- [clamp](https://developer.mozilla.org/zh-CN/docs/Web/CSS/clamp)

## 过滤器函数

- [blur](https://developer.mozilla.org/zh-CN/docs/Web/CSS/filter-function/blur)
- [opacity](https://developer.mozilla.org/zh-CN/docs/Web/CSS/filter-function/opacity)

## 颜色值函数

- [rbg/rgba](https://developer.mozilla.org/zh-CN/docs/Web/CSS/color_value/rgb)
- [hsl/hsla](https://developer.mozilla.org/zh-CN/docs/Web/CSS/color_value/hsl)

## 渐变函数

- [linear-gradient](https://developer.mozilla.org/zh-CN/docs/Web/CSS/gradient/linear-gradient)
- [radial-gradient](https://developer.mozilla.org/zh-CN/docs/Web/CSS/gradient/radial-gradient)
- [conic-gradient](https://developer.mozilla.org/zh-CN/docs/Web/CSS/gradient/conic-gradient)

## 引用函数

- [attr](https://developer.mozilla.org/zh-CN/docs/Web/CSS/attr)
- [env](https://developer.mozilla.org/zh-CN/docs/Web/CSS/env)
- [url](https://developer.mozilla.org/zh-CN/docs/Web/CSS/url)
- [var](https://developer.mozilla.org/zh-CN/docs/Web/CSS/env)

## 网格函数

- [repeat](https://developer.mozilla.org/zh-CN/docs/Web/CSS/repeat)

## 缓动函数

- [steps](https://developer.mozilla.org/zh-CN/docs/Web/CSS/easing-function)
