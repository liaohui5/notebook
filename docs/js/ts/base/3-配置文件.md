...
{
  // "files": [],                                        // 指定需要编译哪些ts文件
  "includes": [], // 指定需要编译哪些ts文件(支持 glob 匹配)
  "excludes": [], // 指定不需要编译哪些ts文件(支持 glob 匹配)
  "compilerOptions": {
    // 编译相关选项
    ...
    "emitDecoratorMetadata": true,                       /* 装饰器源数据支持, 需要安装 Reflect-metadata */
    ...
    // "strictNullChecks": true,                         /* 严格校验 null 和 undefined 空值 */
