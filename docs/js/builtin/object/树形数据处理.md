 ...
 * @param {String|Number} options.root 顶层id的值
 * @param {String} options.idKey 当前数据的 id
 * @param {String} options.pidKey 当前数据的父级 id
 * @param {String} options.childrenKey 当前数据的子级 id
...
function list2tree(data, options = {}) {
  ...
  const { root, idKey, pidKey, childrenKey, cloneSource } = config;
  const target = cloneSource ? JSON.parse(JSON.stringify(data)) : data;
  return target.filter((node) => {
    const subNodes = target.filter((item) => node[idKey] === item[pidKey]);
    if (subNodes.length) {
      node[childrenKey] = subNodes;
    }
    return node[pidKey] === root;
  });
}

const treeData = list2tree(items, {
  root: 0,
  idKey: "id",
  pidKey: "pid",
 ...
 * @param {String} option.childrenKey
 * @returns {Array}
 */
function list2tree(data, options = {}) {
  if (!Array.isArray(data)) {
  ...
  for (let i = 0; i < target.length; i++) {
    ...
    // 获取/创建 子节点 数组
    if (!Array.isArray(dataMap[pid][childrenKey])) {
      dataMap[pid][childrenKey] = [];
    }
    dataMap[pid][childrenKey].push(item);
  }
