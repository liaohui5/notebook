 ...
 * @returns
 */
export function debounce(func, wait = 100, immediateInvok = false, thisArg) {
  let timer;
  let shouldExecute = Boolean(immediateInvok);
...
```js [js]
export function throttle(func, wait = 100, thisArg) {
  let timer;
  let startTime = Date.now();
  return function (...args) {
    timer && clearTimeout(timer);
...
function curry(fn, args) {
  ...
  return function () {
    ...
    var items = args.concat(curArgs); // 可理解为: items = [...args, ...arguments];
    if (items.length < len) {
...
var add = curry(sum);
console.log(add(1, 2, 3)); // 这个不会递归, 直接执行 else 中的代码
 ...
 */
const caches = new WeakSet();
export function once(func, thisArg) {
  return function (...args) {
    if (caches.has(func)) {
      return;
    }
